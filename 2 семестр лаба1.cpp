...
int main() {
    ...
    case '5': {
        int L, length, first, second, ind1 = 0, ind2 = 0;
        ...
        cout << endl << "How do you want to sort your array?" << endl << "1. Increasing   2. Decreasing" << endl;
        cin >> first;

        cout << "What part of the array do you want to sort?" << endl << "1. All of the array" << endl << "2. Sort that part of the array whose elements are between the minimum and maximum value of the array before sorting" << endl << "3. Sort the array in the part between the given sequence numbers by the user" << endl;
        cin >> second;
        ...
        if (second == 3) {
            ...
            cin >> ind2;
            if (ind1 >= length || ind2 >= length || ind1 < 0 || ind2 < 0 || ind1 > ind2) {
                cout << "Invalid input! Please enter valid index values." << endl;
                break;
            }
        }

        int* sorted_arr = Task5(K, L, arr, G, first, second, ind1, ind2);
        for (int i = 0; i < K; i++) {
            cout << sorted_arr[i] << " / ";
        }
